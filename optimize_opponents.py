...
class OptimizeOpponents:
    ...
    def randomOpponentChangeInRounds(self, roundOne : int, roundTwo : int) -> bool:
        ...
        if score < self.currentScore:
            print(f"New score: {score:8.4f}. Swap. Rounds: {roundOne} x {roundTwo}. Players: {playerA} x {playerB}")
    ...
    def randomOpponentChangeInGames(self, gameOne : int, gameTwo : int) -> bool:
        ...
        score = self.scoreFunc()
        if score < self.currentScore:
            print(f"New score: {score:8.4f}. Swap. Games: {gameOne} x {gameTwo}. Players: {playerA} x {playerB}")
    ...
    def scoreFunc(self) -> float:
        ...
        target = 9 * self.schedule.configuration.numAttempts / (self.schedule.configuration.numPlayers-1)
                
        penalty = 0.0
        for playerId in range(self.schedule.numPlayers):
            opponents = metrics.calcPlayerOpponentsHistogram(playerId)
            sd = metrics.calcSquareDeviationExclude(opponents, target, playerId)
            penalty += sd
